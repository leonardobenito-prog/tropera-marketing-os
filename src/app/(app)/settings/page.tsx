...
export default async function SettingsPage({
  ...
  searchParams: { integration_success?: string; integration_error?: string };
}) {
  // Solo Admin puede ver y tocar integraciones — el resto de la app ni
  // siquiera muestra este ítem de forma prominente, pero la ruta se protege igual.
  await requireRole(["ADMIN"]);

  const businessUnits = await prisma.businessUnit.findMany({ orderBy: { name: "asc" } });
  const adAccounts = await prisma.adAccount.findMany({ include: { platform: true, businessUnit: true } });
    ...
    <div className="p-6 space-y-8">
      ...
      <div>
        ...
        <p className="text-xs mt-2" style={{ color: "var(--muted)" }}>
          Hoy están fijos en el código (`execState` en `src/components/ui.tsx`). Para hacerlos editables desde
          aquí, falta una tabla `Settings` de una fila y un formulario que la actualice.
        </p>
      </div>
    </div>
  );
